<div class="col-lg-3 col-md-6">
    <div class="left-area">
        <div class="filter-result-area">
            ...
            <div class="body-area">
                <form id="catalogForm" action="{{ route('front.brand_Category', [Request::route('category')]) }}" method="GET">
                    ...
                    <ul class="filter-list">
                        @foreach ($brands as $element)
                            <li>
                                <div class="content">
                                    <a href="{{route('front.brand_Category', $element->name)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i> {{$element->name}}</a>
                                </div>
                            </li>
                        @endforeach
                    </ul>
        ...
        @if ((!empty($cat) && !empty(json_decode($cat->attributes, true))))
            ...
            <div class="tags-area">
                ...
                <div class="body-area">
                    <form id="attrForm" action="{{route('front.brand_Category', [Request::route('category')])}}" method="post">
                        <ul class="filter">

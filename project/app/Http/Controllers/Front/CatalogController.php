...
class CatalogController extends Controller
{
    ...
    public function discount_Category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
        $cat = null;
        $subcat = null;
        $childcat = null;
        ...
        $prods = Product::where('is_discount', '1')
        ...
        return view('front.discount-category', $data);
    }
    ...
    public function brand_Category(Request $request, $slug=null)
    {
        $cat = null;
        $minprice = $request->min;
        $maxprice = $request->max;
        ...
        if (!empty($slug)) {
            $cat = Partner::where('name', $slug)->firstOrFail();
            $data['cat'] = $cat;
        }

        $prods = Product::whereNotNull('brand_name')
            ->when($cat, function ($query, $cat) {
                return $query->where('brand_name', $cat->name);
            })
            ->when($search, function ($query, $search) {
                return $query->whereRaw('MATCH (name) AGAINST (? IN BOOLEAN MODE)' , array($search));
            })
...
//            }
        ...
        return view('front.brand-category', $data);
    }
    ...
    public function product($slug)
    {
        ...
        {
            $curr = Currency::where('name', '=', Session::get('currency'))->first();
        }
        else
        {
        ...
        if ($productt->policy == "<br>") {
            $productt->policy = "There is no BUY & RETURN POLICY";
        }
        if ($productt->details == "<br>") {
            $productt->details = "There is no DESCRIPTION";
        }
